...
#define T39FX_USES_STATIC_BUFFER
...
#ifdef T39FX_USES_STATIC_BUFFER
...
int broadcast_drv_if_power_on(void)
{
	...
	if(gpMPI_Buffer == NULL)
	{
#ifndef T39FX_USES_STATIC_BUFFER
	...
	retval = tunerbb_drv_t39fx_power_on( );

	if(retval == TRUE)
	{
		rc = OK;
	}
	//tunerbb_drv_t39fx_set_userstop( 1 );
...
int broadcast_drv_if_power_off(void)
{
	...
	retval = tunerbb_drv_t39fx_power_off( );

	if(retval == TRUE)
	{
		rc = OK;
	}
	//tunerbb_drv_t39fx_set_userstop( 0 );
...
int broadcast_drv_if_set_channel(unsigned int freq_num, unsigned int subch_id, unsigned int op_mode)
{
	...
	retval = tunerbb_drv_t39fx_set_channel(freq_num, subch_id, op_mode);
...
int broadcast_drv_if_detect_sync(int op_mode)
{
	...
	retval = tunerbb_drv_t39fx_re_syncdetector(op_mode);
...
int broadcast_drv_if_get_sig_info(struct broadcast_tdmb_sig_info *dmb_bb_info)
{
	...
	retval = tunerbb_drv_t39fx_get_ber(dmb_bb_info);
...
int broadcast_drv_if_get_ch_info(char* buffer, unsigned int* buffer_size)
{
	...
	retval = tunerbb_drv_t39fx_get_fic(buffer, buffer_size);
...
int broadcast_drv_if_reset_ch(void)
{
	...
	retval = tunerbb_drv_t39fx_reset_ch( );
...
{
	tunerbb_drv_t39fx_set_userstop( mode );
	return OK ;
}

int broadcast_drv_if_select_antenna(unsigned int sel)
{
	tunerbb_drv_t39fx_select_antenna(sel);
	return OK;
}
